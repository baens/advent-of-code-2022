...
val startLocation = map.findStartLocation()
val endLocation = map.findEndLocation()

val shortestPath = search()
...
fun search():List<Point> {
  ...
  while(openVertices.size > 0) {
    ...
    val currentHeight = when(val height = map[currentLocation.second][currentLocation.first]) {
      START -> heights["a"] ?: throw Exception("Cant find start height")
      else -> height
    }
...
fun List<List<Int>>.findStartLocation(): Point = this.findItem(START)
